...
#     # Set up datasets
#     test_dataset = GeneratedMeshDataset(n_fibers=n_fibers, directory=Path(test_dir), subdivide=mesh_subdivide, kappa=kappa)

#     # Set up dataloaders
#     test_loader = DataLoader(test_dataset, batch_size=batch_size)

#     n_mesh = test_dataset.n_mesh
#     sphere = test_dataset.sphere
#     device = torch.device(f"cuda:{gpu_id}" if torch.cuda.is_available() else "cpu")
...
#     print(model)
#     pdf_mesh, total_odf_mesh = next(iter(test_loader))
#     print(f"Input shape: {total_odf_mesh.shape}")
#     print(f"Output shape: {model(total_odf_mesh).shape}")
#     print(model(total_odf_mesh))
...
#     # Test model
#     model.load_state_dict(torch.load(save_dir / f"best_model.pth", weights_only=True))
#     model.eval()
#     with torch.no_grad():
#         test_loss = 0
#         for test_data in tqdm(test_loader, total=len(test_loader)):
#             pdf_mesh, total_odf_mesh = test_data
#             pdf_mesh = pdf_mesh.to(device)
#             total_odf_mesh = total_odf_mesh.to(device)

#             output = model(total_odf_mesh)
#             test_loss += criterion(output, pdf_mesh).item()
...
#         img_path = save_dir / "test_mesh_plots.png"
#         fig.savefig(img_path)
#         plt.close(fig)
